...
import { FiMic, FiMicOff, FiVolume2, FiVolumeX, FiTrash2 } from "react-icons/fi";
...
export function StreamOverlay({ device }: VideoOverlayProps) {
  ...
  function onDeleteCamera(event: MouseEvent) {
    if (user && device) {
      removeItem(`users/${user.uid}/cameras/${device.id}`)
      dispatch(ConnectionActionCreator.removeRemoteDevice(device.id))
    }
  }
  ...
  return (<div className="stream-overlay container-fluid position-absolute top-0 start-0 z-3 px-0" onClick={onClick}>
    <div className="row gx-1 m-1">
      ...
      {localDevice?.data()?.deviceType === "viewer"
        ? <div className="col-auto">
          <button className="icon-button btn btn-outline-primary" onClick={onDeleteCamera}>
